...
class RequestValidationAgent(BaseAgent):
    def run(self, task: str) -> Dict[str, Any]:
        ...
        {{"is_valid": true/false, "reasons": ["список причин", "если задание не валидно"]}}
        ...
        try:
            ...
            return self._format_result({"is_valid": False, "reasons": ["Ошибка парсинга ответа LLM"]}, 0.1)
        except Exception as e:
            logger.error(f"Непредвиденная ошибка в RequestValidationAgent: {str(e)}")
            return self._format_result({"is_valid": False, "reasons": [str(e)]}, 0.1)
